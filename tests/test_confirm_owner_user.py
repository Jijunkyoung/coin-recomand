...
class ConfirmOwnerUserTests(unittest.TestCase):
    ...
    @patch("scripts.confirm_owner_user.urlopen")
    def test_reports_only_safe_account_counts_when_owner_does_not_match(self, mock_urlopen):
        mock_urlopen.return_value = response({"users": [
            {"id": "other-1", "email": "other@example.com", "email_confirmed_at": None},
            {"id": "other-2", "email": "confirmed@example.com", "email_confirmed_at": "2026-09-24T00:00:00Z"},
        ]})

        with self.assertRaisesRegex(RuntimeError, r"total users: 2, unconfirmed users: 1"):
            confirm_owner(ENV)
