...
대시보드 입력란과 `브라우저 목록만 저장`은 주소 작성·복사를 돕는 로컬 기능이며 실제 발송 설정을 직접 변경하지 않습니다. 실제 주소는 `EMAIL_TO 값 복사` 후 `실제 발송주소 변경`에서 기존 `EMAIL_TO`의 **Update secret → Save changes** 순서로 수정해야 합니다. `테스트 메일 보내기`는 인증정보를 공개 페이지에 노출하지 않고 GitHub Actions 실행 화면을 열며, `Run workflow`에서 `메일 발송`을 체크하면 현재 GitHub Secret에 등록된 주소로 최신 보고서를 한 번 발송합니다.
