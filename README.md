...
대시보드의 `테스트 메일 보내기` 버튼은 인증정보를 공개 페이지에 노출하지 않고 GitHub Actions 실행 화면을 엽니다. `Run workflow`에서 `메일 발송`을 체크해 실행하면 현재 `EMAIL_TO` Secret에 등록된 주소로 최신 보고서를 한 번 발송합니다. 체크하지 않으면 분석·배포만 수행합니다.
